...
export class AuthService {
  ...
  register(registerData){
    ...
    'Content-Type': 'application/json'});
    const options = { headers };
    return this.http.post<any>(this.registerUrl, JSON.stringify(registerData), options);
  }
  openPopup() {
  ...
  isAdminLoggedIn() {
    return !(localStorage.getItem('token') && (helper.decodeToken(localStorage.getItem('token')).role[0] === 'user'));
  }
  startTest() {
